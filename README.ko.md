...
- [`weekly-report-writer`](.claude/agents/weekly-report-writer.md) — 해외 연구소 보고, 메모, PR 제목+링크, 소스 폴더, KPI 테이블처럼 섞여 있는 근거를 읽어 관리층 공유용 주간 보고로 정리합니다. 특히 임원/중간관리자 보고처럼 지표, 협의 성과, 이슈, 근거 기반 문체가 중요한 상황에 맞고, 자료가 덜 준비된 경우에는 한 번에 몰아 묻지 않고 인터뷰하듯 순차적으로 정보를 받아 정리할 수 있습니다. 주차를 따로 주지 않으면 에이전트 실행일 기준 해당 주 월요일~금요일을 금주 보고 구간으로 자동 해석하며, 활성 사용자 수뿐 아니라 활성 세션 같은 운영 지표도 다룰 수 있습니다.
...
자료가 아직 다 정리되지 않았다면, 한꺼번에 템플릿을 채우지 않아도 됩니다. 에이전트가 필요한 정보를 질문 하나씩 순차적으로 받아가며 보고 본문을 완성할 수 있습니다.

보고 기간을 따로 지정하지 않으면, 에이전트는 실행일 기준 해당 주의 월요일부터 금요일까지를 기본 금주 보고 구간으로 사용합니다.

지표 표는 별도 언급이 없으면 금주가 아니라 지난주 확정 수치(`W-1`)를 기본으로 사용합니다. 즉, 본문은 금주 진행사항을 다루고 지표는 지난주 기준으로 붙입니다.

원천 자료에 더 큰 프로그램명이나 상위 과제명이 나오더라도, 최종 보고서에는 사용자가 지정한 소과제 표시명으로 정리할 수 있습니다.

이슈 사항은 경고성 표현보다 관리 포인트 중심으로 정리합니다. 예를 들어 `문제가 크다`보다 `검토 필요`, `부담 존재`, `영향 가능성 존재`, `대응 진행 중` 같은 표현을 우선합니다.

문장이 너무 기술 구현 중심으로 길어지면, 구현 설명보다 결과와 효과를 먼저 앞세워 더 쉽게 읽히는 형태로 다시 씁니다.

해외 연구소 보고는 모든 항목을 그대로 요약하지 않고, 해외연의 의미 있는 기여와 역량이 드러나는 항목을 중심으로 선별합니다. 판단이 모호한 항목은 임의로 포함하거나 제외하지 않고, 사용자가 가감할 수 있도록 후보로 드러냅니다.
